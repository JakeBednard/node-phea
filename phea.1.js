...
async function getGroup(groupId, ipAddress, username) {

    let groups = await HueApi.getGroup(groupId, ipAddress, username);
...
async function createEngine(options) {
    ...
    let pheaEngine = new PheaEngine(opts);
    
    await pheaEngine.start();

    return PheaEngine;

}

async function destroyEngine(pheaEngine) {
    await pheaEngine.stop();
    return;
}

async function transition(lights=[], rgb=[0,0,0], tweenTime=0, block=false) {
...
function _checkTransitionOptions(lights, rgb, tweenTime, block) {
...
function _checkInstanceOptions(opts) {
    ...
    if (opts.group && typeof(opts.group) !== 'number') throw new Error(
        "PHEA [Configuration]: 'group' must be of type int between 0 and 255 inclusive."
    )
    ;
    if (opts.group && opts.group < 0 || opts.group > 255)  throw new Error(
